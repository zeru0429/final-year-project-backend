...
const childController ={
   ...
   update: async (req:Request,res:Response,next:NextFunction)=>{
      ...
      const isChildExist = await prisma.childrens.findFirst({where: {id: +req.childId}});
      if(!isChildExist){
        return next(new UnprocessableEntity('Child not found',404,ErrorCode.CHILD_NOT_FOUND,null));
      }
   ...
   delete: async (req:Request,res:Response,next:NextFunction)=>{
      ...
      const deletedChild = await prisma.childrens.delete({where:{id: +req.childId}});
      res.status(200).json({
