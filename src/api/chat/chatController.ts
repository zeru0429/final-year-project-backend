...
const chatController = {
   ...
   deleteGroupChat: async (req:Request, res:Response, next:NextFunction) =>{
      ...
      // check if the user who is deleting is the group admin
       if(chat.adminId!.toString() !== req.user?.id.toString()) return next(new UnprocessableEntity('Only group admin can delete the group chat', 401, ErrorCode.UNPROCESSABLE, null));
       // delete the chat
       ...
       chat?.participants?.forEach((participant) => {
         if (participant.id.toString() === req.user?.id.toString()) return; // don't emit the event for the logged in use as he is the one who is deleting
